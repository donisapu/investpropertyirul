...
export default function OurVilla({ auth, villa = [], sliders = [], landings }) {
    // 1. STATE & LOGIKA SLIDER (Tambahan Perbaikan)
    const [i, setI] = useState(0);

    // Ambil array gambar dari prop sliders atau fallback ke gambar default
    const activeImages = Array.isArray(sliders) && sliders.length > 0
        ? sliders.map((item) => item.image || item)
        : [
            "/images/default-villa-1.jpg",
            "/images/default-villa-2.jpg",
        ];

    const next = () => {
        setI((prevI) => (prevI + 1) % activeImages.length);
    };

    const prev = () => {
        setI((prevI) => (prevI - 1 + activeImages.length) % activeImages.length);
    };
