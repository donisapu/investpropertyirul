...
import { Bed, Bath, Maximize, Home, MapPin, ChevronRight, ChevronLeft, Info, CheckCircle, ExternalLink, X } from 'lucide-react';
...
export default function Show({ property }) {
    ...
    const leaderboard = [
        { id: 1, name: 'k3ptian', tokens: 50, avatar: 'https://i.pravatar.cc/150?u=1' },
        { id: 2, name: 'borkangvilla', tokens: 120, avatar: 'https://i.pravatar.cc/150?u=2', rank: 1 },
        { id: 3, name: '0xdansk', tokens: 85, avatar: 'https://i.pravatar.cc/150?u=3' },
        ...
        <PublicLayout>
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                ...
                <div className="flex items-center text-sm text-slate-500 mb-6">
                    <Link href="/investments" className="hover:text-emerald-600">Investments</Link>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className="lg:col-span-2 space-y-8">
                            ...
                            <div className="grid grid-cols-4 gap-4">
                                ...
                                >
                                    ...
                                    <div className="absolute top-4 left-4">
                                        <span className={`px-3 py-1 rounded-lg text-sm font-bold shadow-sm ${property.sold ? 'bg-slate-800 text-white' : 'bg-lime-400 text-slate-900'}`}>
                                            {property.sold ? 'Sold Out' : 'Available'}
                                        </span>
                                    </div>
                        ...
                        <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-200">
                            <h3 className="text-lg font-bold text-slate-900 mb-6 text-center">Monthly Annualized Net Rental Yields</h3>
                            <div className="h-[300px] w-full">
                                <ResponsiveContainer width="100%" height="100%">
                                    <AreaChart data={chartData}>
                                        ...
                                        <XAxis dataKey="name" axisLine={false} tickLine={false} tick={{ fontSize: 12, fill: '#64748b' }} interval={1} />
                                        <YAxis axisLine={false} tickLine={false} tick={{ fontSize: 12, fill: '#64748b' }} unit="%" />
                                        <Tooltip
                                            contentStyle={{ borderRadius: '8px', border: 'none', boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0.1)' }}
                                            formatter={(value) => [`${value}%`, 'Yield']}
                                        />
                                        <Area type="monotone" dataKey="returns" stroke="#059669" strokeWidth={2} fillOpacity={1} fill="url(#colorReturns)" />
                                        <Area type="monotone" dataKey="avg" stroke="#94a3b8" strokeDasharray="5 5" fill="none" />
                                    </AreaChart>
                                </ResponsiveContainer>
                            </div>
                        </div>
                        ...
                        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                            <div className="border-b border-slate-200">
                                <nav className="flex overflow-x-auto">
                                    {['Details', 'Financials', 'Documents', 'Market', 'Timeline'].map((tab) => (
                                        <button
                                            key={tab}
                            ...
                            <div className="p-6">
                                {activeTab === 'details' && (
                                    <div className="space-y-6">
                                        <div>
                                            <h3 className="text-lg font-bold text-emerald-800 mb-3">About the Property</h3>
                                        ...
                                        {property.map_url && (
                                            <div>
                                                <h3 className="text-lg font-bold text-emerald-800 mb-3">Location</h3>
                                ...
                                {activeTab === 'financials' && (
                                    <div className="space-y-6">
                                        <div>
                                            <h3 className="text-lg font-bold text-emerald-800 mb-3">Financial Overview</h3>
                                            <div className="prose prose-slate max-w-none text-slate-600">
                                                {property.financial ? (
                                                    <p className="whitespace-pre-line">{property.financial}</p>
                                                ) : (
                                                    <p className="text-slate-400 italic">No financial details available.</p>
                                                )}
                                            </div>
                                        </div>
                                    </div>
                                ...
                                {activeTab === 'market' && (
                                    <div className="space-y-6">
                                        <div>
                                            <h3 className="text-lg font-bold text-emerald-800 mb-3">Market Analysis</h3>
                                            <div className="prose prose-slate max-w-none text-slate-600">
                                                {property.market ? (
                                                    <p className="whitespace-pre-line">{property.market}</p>
                                                ) : (
                                                    <p className="text-slate-400 italic">No market analysis available.</p>
                                                )}
                                            </div>
                                        </div>
                                    </div>
                                ...
                                {activeTab === 'timeline' && (
                                    <div className="space-y-6">
                                        <div>
                                            <h3 className="text-lg font-bold text-emerald-800 mb-3">Project Timeline</h3>
                                            <div className="prose prose-slate max-w-none text-slate-600">
                                                {property.timeline ? (
                                                    <p className="whitespace-pre-line">{property.timeline}</p>
                                                ) : (
                                                    <p className="text-slate-400 italic">No timeline available.</p>
                                                )}
                                            </div>
                                        </div>
                                    </div>
                                )}
                                {!['details', 'financials', 'market', 'timeline'].includes(activeTab) && (
                    ...
                    <div className="lg:col-span-1 space-y-6">
                        ...
                        <div className="bg-white z-50 rounded-2xl p-6 shadow-sm border border-slate-200 sticky top-24">
                            <div className="flex justify-between items-center mb-2">
                                <span className="text-emerald-700 font-bold bg-emerald-50 px-3 py-1 rounded-full text-xs">{property.financials.progress}% Funded</span>
                                <span className="text-slate-500 text-xs font-medium">{property.financials.tokens_left.toLocaleString()} / {property.financials.total_tokens.toLocaleString()} tokens left</span>
                            </div>
                            <div className="w-full bg-slate-100 rounded-full h-2 mb-6">
                                <div className="bg-emerald-500 h-2 rounded-full" style={{ width: `${Math.max(property.financials.progress, 2)}%` }}></div>
                            </div>

                            <div className="grid grid-cols-2 gap-2 mb-6 text-center">
                                <div className="p-2 rounded-lg bg-slate-50">
                                    <div className="text-emerald-600 font-bold text-lg">{property.financials.irr}</div>
                                ...
                                <div className="p-2 rounded-lg bg-slate-50">
                                    <div className="text-slate-900 font-bold text-lg">{property.financials.roi_period} Months</div>
                                    <div className="text-[0.6rem] text-slate-500 font-bold uppercase flex items-center justify-center gap-1">
                            ...
                            <div className="grid grid-cols-2 gap-3 mb-6">
                                <button className={`w-full py-3 rounded-lg font-bold text-white mb-3 shadow-lg ${property.sold ? 'bg-slate-400 cursor-not-allowed' : 'bg-emerald-800 hover:bg-emerald-700 shadow-emerald-900/20'}`}>
                            ...
                            <div className="text-center text-xs text-slate-500">
                                If you invest 1,000 tokens, you could earn an estimated annual return of:
                                <div className="text-slate-900 font-bold text-lg mt-1">IDR 958,000</div>
                            </div>
                        </div>
                        ...
                        <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-200">
                            <div className="text-center mb-6">
                                <h3 className="text-sm font-bold text-slate-500 uppercase tracking-wider mb-4 flex items-center justify-center gap-2">
                                    <span className="text-amber-400">👑</span> Token Holders Leaderboard
                                </h3>
                                <div className="mb-14 font-bold text-slate-900 text-lg">{property.name}</div>
                            </div>
                            ...
                            <div className="flex items-end justify-center gap-4 mb-8 h-32">
                                ...
                                <div className="flex flex-col items-center">
                                    ...
                                    <div className="text-[0.6rem] font-bold text-slate-900">{leaderboard[2].name}</div>
                                    <div className="h-12 w-12 bg-slate-100 rounded-t-lg flex items-center justify-center font-bold text-slate-400 text-sm mt-1">2</div>
                                </div>
                                {/* 1st Place */}
                                <div className="flex flex-col items-center z-10">
                                    ...
                                    <div className="text-xs font-bold text-slate-900">{leaderboard[1].name}</div>
                                    <div className="text-[0.6rem] text-emerald-600 font-bold mb-1">ID: {leaderboard[1].id}</div>
                                    <div className="h-16 w-16 bg-gradient-to-b from-amber-100 to-white rounded-t-lg flex items-center justify-center font-bold text-amber-500 text-xl mt-1 border-t border-amber-200">1</div>
                                </div>
                                {/* 3rd Place */}
                                <div className="flex flex-col items-center">
                                    <div className="w-10 h-10 rounded-full border-2 border-slate-200 overflow-hidden mb-1">
                                    ...
                                    <div className="text-[0.6rem] font-bold text-slate-900">{leaderboard[0].name}</div>
                                    <div className="h-8 w-12 bg-slate-50 rounded-t-lg flex items-center justify-center font-bold text-slate-400 text-sm mt-1">3</div>
                                </div>
                            </div>
                                ...
                                {leaderboard.slice(3).map((user, idx) => (
                                    <div key={user.id} className="flex items-center justify-between text-sm">
                                        <div className="flex items-center gap-3">
                                            <div className="font-bold text-slate-400 w-4">{idx + 4}.</div>
                                        ...
                                        <div className="text-slate-400 text-xs">GORO ID: {user.id * 1234}</div>
                                    </div>
                                ))}
                            </div>
                        </div>
